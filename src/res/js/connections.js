var drawConnections = function (svg, diagram, connections, icons, notes) {
    var connectionLabelFontSize = Math.min(diagram.xBand.UnitStep / 15, diagram.yBand.UnitStep / 15)
    connections.forEach(function (connection, index) {
        ...
        if (curve == d3.curveStepBefore) { startOffset = diagram.yBand.UnitStep / 2 }
        if (curve == d3.curveStepAfter) { startOffset = diagram.xBand.UnitStep / 2 }
        if (curve == d3.curveStep) { startOffset = diagram.xBand.UnitStep / 2 }
        if (curve == d3.curveLinear) {
            // find the angle of the center to the corner
            var c2cRadians = Math.atan2(diagram.yBand.UnitStep / 2, diagram.xBand.UnitStep / 2);
            var c2cDegrees = c2cRadians * 180 / Math.PI
            var A = Math.abs(c2cDegrees - Math.abs(angleDegrees))
            var C = 90 - c2cDegrees
            if (Math.abs(angleDegrees) > C) { C = 90 - C }
            var B = 180 - (A + C)
            var b = Math.sqrt(Math.pow(diagram.xBand.UnitStep / 2, 2) + Math.pow(diagram.yBand.UnitStep / 2, 2))
        ...
        if (curve == d3.curveStepBefore) {
            startOffset = diagram.xBand.UnitStep / 2
        } else if (curve == d3.curveStepAfter) {
            startOffset = diagram.yBand.UnitStep / 2
        }
